...
def train(args):
    ...
    while maacktr.n_episodes < MAX_EPISODES:
        # print(maacktr.n_episodes)
        maacktr.interact()
        if maacktr.n_episodes >= EPISODES_BEFORE_TRAIN:
            # print("------------------------start training------------------")
            maacktr.train()
        if maacktr.episode_done and ((maacktr.n_episodes + 1) % EVAL_INTERVAL == 0):
            rewards, _, _, avg_s = maacktr.evaluation(env_eval, dirs['train_videos'], EVAL_EPISODES)
            rewards_mu, rewards_std = agg_double_list(rewards)
            # rewards_mu = np.mean(rewards)
            print("Episode %d, Average Reward %.2f, Average Speed %.2f" % (maacktr.n_episodes + 1, rewards_mu, np.mean(avg_s)))
            eval_rewards.append(rewards_mu)
            speed.append(np.mean(avg_s))
            # save the model
            maacktr.save(dirs['models'], maacktr.n_episodes + 1)

    # save the model
    maacktr.save(dirs['models'], MAX_EPISODES + 2)
    np.savetxt(output_dir+'/rewards.txt', eval_rewards)
    np.savetxt(output_dir+'/speeds.txt', speed)
    # plt.figure()
...
def evaluate(args):
    ...
    rewards, _, steps, avg_speeds, crash = maacktr.evaluation(env, video_dir, len(seeds), is_train=False)
    rewards_mu, rewards_std = agg_double_list(rewards)
    # rewards_mu = np.mean(rewards)
    print("Average Speed %d, Average Reward %.2f, Crash %.2f" % (np.mean(avg_speeds), rewards_mu, np.mean(crash)))
