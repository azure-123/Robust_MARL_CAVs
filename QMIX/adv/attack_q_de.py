...
def predict_classes(xs, img, learner, actions, state_batch, agents_available_actions): # 扰动后的动作的MIX_Value值
    imgs_perturbed = perturb_actions(xs, img.clone())

    # action_values[agents_available_actions == 0] = 10000000.0
    # actions = actions.repeat(len(xs), 1,1)
    # 扰动后的动作
	
    # value_perturbed = torch.gather(action_values, dim=2, index=imgs_perturbed).squeeze(0)#.view(-1,n_actor, n_agents, )

    input = Variable(imgs_perturbed, volatile=True)#.no_grad()#.cuda()
	# print(value_perturbed.size())
	# print(torch.FloatTensor(state_batch).repeat(len(xs), 1).size())
    # print(input)
    # print(torch.FloatTensor(state_batch))
    predictions = learner.mix_value(input, torch.FloatTensor(state_batch).repeat(len(xs), 1,1))
    predictions = predictions.data.cpu().numpy()
    return predictions #if minimize else 1 - predictions

def attack_success(x, img, target_calss, learner, actions, state_batch, verbose=False): # 看攻击是否成功
    ...
    # print(input)
    # print(torch.FloatTensor(state_batch).unsqueeze(0))
	# aaa
    q_tot = learner.mix_value(input, torch.FloatTensor(state_batch).unsqueeze(0)).data.cpu().numpy()[0][0][0]
...
# learner: qmix 网络
# pixels：被攻击智能体的数量
def attack_q_de(img, label, learner, actions,n_agent, n_action, state_batch,agents_available_actions,
    ...
    predict_fn = lambda xs: predict_classes(
		xs, img, learner, actions,  state_batch, agents_available_actions) # 要最小化的目标函数
    # x, img, target_calss, learner, action_values, state_batch, verbose=False
    callback_fn = lambda x, convergence: attack_success(
		x, img, target_calss, learner, actions, state_batch, verbose)
    # callback_fn = None
