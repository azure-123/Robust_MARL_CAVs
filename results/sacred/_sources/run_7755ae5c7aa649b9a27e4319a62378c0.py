...
from utils1.logging import Logger
from utils1.timehelper import time_left, time_str
...
def evaluate_sequential(args, runner, learner, logger):
    ...
    #long running
    #do something other
    seeds = [i for i in range(0, 600, 20)]

    for _ in range(len(seeds)):
        args.env_args['seed'] = seeds[_]
        print(args.env_args['seed'])
        adv_test = False
        if args.Number_attack > 0:
            adv_test = True
        print(_)
        runner.run(test_mode=_+1, learner = learner, adv_test = adv_test)
    ...
    while runner.t_env <= args.t_max:
        if args.Number_attack > 0 :
            ...
            if (runner.t_env - last_test_T) / args.test_interval >= 1.0:
                ...
                last_test_T = runner.t_env
                for _ in range(n_test_runs):
                    runner.run(test_mode=_+1, learner = learner, adv_test = False)
